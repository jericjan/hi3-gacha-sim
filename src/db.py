...
class Database:
    ...
    def show_all(self):        
        ...
        print(f"{'ID'.center(5)} | {'Name'.center(30)} | {'Rounds'.center(7)} | {'Valk Pity'.center(9)} | {'Gear Pity'.center(9)}")
        for row in rows:
            print(f"{str(row[0]).center(5)} | {str(row[1]).center(30)} | {str(row[2]).center(7)} | {str(row[3]).center(9)} | {str(row[4]).center(9)}")
        cursor.close()
        self.conn.commit()     
